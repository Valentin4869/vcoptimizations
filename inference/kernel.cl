...
__kernel void rowConv(__global float* d_Src, __global float *d_Dst, __constant float *c_rowKernel) {
	...
	const int block_idx = (ROWS_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH)+get_group_id(0)*ROWS_BLOCKDIM_X + lix;
	const int dst_idx = block_idx + (get_group_id(2))*(WIDTH)*(HEIGHT); //global index 
	d_Src += (ROWS_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH)+get_group_id(0)*ROWS_BLOCKDIM_X;
	...
	for (int c = 0; c < K1; c++) {
		...
		//	C_sum += c_rowKernel[KERNEL_RADIUS - j + c*KERNEL_LENGTH + get_group_id(2)*KERNEL_LENGTH*C2] * l_data[liy][lix + j + KERNEL_RADIUS];
		...
		for (int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++) {
			...
			C_sum += c_rowKernel[KERNEL_RADIUS + j + c*KERNEL_LENGTH + get_group_id(2)*KERNEL_LENGTH*K1] * l_data[liy][lix + j + KERNEL_RADIUS];
...
__kernel void colConv(
	__global float *d_Src,
	__global float *d_Dst,
	__constant float *c_colKernel) {
	...
	const int liy = get_local_id(1);
	const int giy = get_group_id(1)*get_local_size(1) + get_local_id(1);
	const int block_idx = (COLUMNS_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH)+get_group_id(0)*COLUMNS_BLOCKDIM_X + lix;
	const int dst_idx = block_idx + (get_group_id(2))*(WIDTH)*(HEIGHT);
	d_Src += (COLUMNS_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH)+get_group_id(0)*COLUMNS_BLOCKDIM_X;
	...
	for (int c = 0; c < C1; c++) {
		...
		if (get_group_id(1) == 0) {
			...
			if (get_local_id(1) < KERNEL_RADIUS)
				...
				l_data[liy + COLUMNS_BLOCKDIM_Y + KERNEL_RADIUS][lix] = d_Src[lix + COLUMNS_BLOCKDIM_Y*WIDTH + c*WIDTH  * HEIGHT];
			...
			if (get_local_id(1) < KERNEL_RADIUS * 2)
				...
				l_data[liy + COLUMNS_BLOCKDIM_Y][lix] = d_Src[lix - KERNEL_RADIUS*WIDTH + COLUMNS_BLOCKDIM_Y*WIDTH + c*WIDTH * HEIGHT];
		}
		...
		/*for (int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++) {
			...
			C_sum += c_colKernel[KERNEL_RADIUS - j + c*KERNEL_LENGTH + get_group_id(2)*KERNEL_LENGTH*C1] * l_data[liy + j + KERNEL_RADIUS][lix];
...
__kernel void rowConv2(
	...
	const int liy = get_local_id(1);
	const int giy = get_group_id(1)*get_local_size(1) + get_local_id(1);
	const int block_idx = (ROWS_2_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH / 2) + get_group_id(0)*ROWS_2_BLOCKDIM_X + lix;
	const int dst_idx = block_idx + (get_group_id(2))*(WIDTH / 2)*(HEIGHT / 2); //global index 

	d_Src += (ROWS_2_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH / 2) + get_group_id(0)*ROWS_2_BLOCKDIM_X;
	...
	for (int c = 0; c < K2; c++) {
		...
		//	C_sum += c_row2Kernel[KERNEL_RADIUS - j + c*KERNEL_LENGTH + get_group_id(2)*KERNEL_LENGTH*C2] * l_data[liy][lix + j + KERNEL_RADIUS];
		...
		for (int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++) {
			...
			C_sum += c_row2Kernel[KERNEL_RADIUS + j + c*KERNEL_LENGTH + get_group_id(2)*KERNEL_LENGTH*K2] * l_data[liy][lix + j + KERNEL_RADIUS];
...
__kernel void colConv2(
	...
	const int liy = get_local_id(1);
	const int giy = get_group_id(1)*get_local_size(1) + get_local_id(1);
	const int block_idx = (COLUMNS_2_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH / 2) + get_group_id(0)*COLUMNS_2_BLOCKDIM_X + lix; //global index 
	const int dst_idx = block_idx + (get_group_id(2))*(WIDTH / 2)*(HEIGHT / 2); //global index 
	d_Src += (COLUMNS_2_BLOCKDIM_Y*get_group_id(1) + liy)*(WIDTH / 2) + get_group_id(0)*COLUMNS_2_BLOCKDIM_X;
	...
	for (int c = 0; c < N1; c++) {
		...
		if (get_group_id(1) == 0) {
			...
			if (get_local_id(1) < KERNEL_RADIUS)
				...
				l_data[liy + COLUMNS_2_BLOCKDIM_Y + KERNEL_RADIUS][lix] = d_Src[lix + COLUMNS_2_BLOCKDIM_Y*WIDTH / 2 + c*WIDTH / 2 * HEIGHT / 2];
			...
			if (get_local_id(1) < KERNEL_RADIUS * 2)
				...
				l_data[liy + COLUMNS_2_BLOCKDIM_Y][lix] = d_Src[lix - KERNEL_RADIUS*WIDTH / 2 + COLUMNS_2_BLOCKDIM_Y*WIDTH / 2 + c*WIDTH / 2 * HEIGHT / 2];
		}
		...
		//	C_sum += c_col2Kernel[KERNEL_RADIUS - j + c*KERNEL_LENGTH + get_group_id(2)*KERNEL_LENGTH*C3] * l_data[liy + j + KERNEL_RADIUS][lix];
		...
		for (int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++) {
			...
			C_sum += c_col2Kernel[KERNEL_RADIUS + j + c*KERNEL_LENGTH + get_group_id(2)*KERNEL_LENGTH*N1] * l_data[liy + j + KERNEL_RADIUS][lix];
...
__kernel void MaxPool1(__global const float* src, __global float* dst) {
	...
	dst += get_group_id(0) * get_local_size(0) / 2 + get_group_id(1) * WIDTH / 2 * get_local_size(1) / 2 + get_group_id(2) * WIDTH / 2 * WIDTH / 2;
	src += get_group_id(0) * get_local_size(0) + get_group_id(1)*get_local_size(1) * WIDTH + get_group_id(2) * WIDTH*HEIGHT;
...
__kernel void MaxPool2(__global const float* src, __global float* dst) {
	...
	dst += get_group_id(0) * get_local_size(0) / 2 + get_group_id(1) * WIDTH / 4 * get_local_size(1) / 2 + get_group_id(2) * WIDTH / 4 * HEIGHT / 4;
	src += get_group_id(0) * get_local_size(0) + get_group_id(1)*get_local_size(1) * WIDTH / 2 + get_group_id(2) *  WIDTH / 2 * HEIGHT / 2;
